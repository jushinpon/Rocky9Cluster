...
system("./configure --prefix=$prefixPath --enable-fast=all,O3 FFLAGS=-fallow-argument-mismatch FCFLAGS=-fallow-argument-mismatch");# --with-slurm=[/usr/local] VERBOSE=1 |tee 00mpich_configure.txt"); #./configure
